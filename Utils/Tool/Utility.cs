...
namespace Utils.Tool
{
    ...
    public class Utility : Page
    {
        ...
        public static string HtFromPageForDiscount(IIdentity _identity)
        {
            var request = HttpContext.Current.Request;             
            request.InputStream.Position = 0;//核心代码
            byte[] byts = new byte[request.InputStream.Length];
        ...
        /// <returns></returns>
        public static string[] RecordFromPage(IIdentity _identity,string recorderlist= "RecordIDList")
        {
            string data = getpageValue();
            Hashtable ht = new Hashtable();
            string RecordIDList = "";
            string[] RecordId = { };
            if (!string.IsNullOrEmpty(data))
        ...
        /// <param name="recorderlist"></param>
        /// <returns></returns>
        public static string[] GetRecordIDList(IIdentity _identity, string recorderlist = "RecordIDList")
        {
            string data = getpageValue();
            string[] RecordId = { };
            if (!string.IsNullOrEmpty(data))
            {
                ...
                if (!string.IsNullOrEmpty(data))
                {
                    foreach (JProperty item in oData.Children())
                    {
                        if (item.Name == recorderlist)
                        {
                            RecordId = Newtonsoft.Json.JsonConvert.DeserializeObject<string[]>(item.Value.ToString());
                            break;
                        }
                    }
                }
        ...
        public static string getpageValue()
        {
            var request = HttpContext.Current.Request;
            //request.ContentEncoding("UTF-8");
            request.InputStream.Position = 0;//核心代码
        ...
        public static string GetLanguageList(IIdentity _identity)
        {
            var request = HttpContext.Current.Request;
